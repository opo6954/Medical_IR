...
int main()
{
	...
	*/
	string lesion_image_ac = "", lesion_text_ac = "";

	for (int i = 0; i<6; i++)
	{
		lesion_image_ac += lesion_image[i];
		lesion_image_ac.push_back(' ');
		lesion_text_ac += lesion_text[i];
		lesion_text_ac.push_back(' ');
	}

	std::vector<char> writable1(lesion_image_ac.begin(), lesion_image_ac.end());
	writable1.push_back('\0');
	char* lesion_image_acc = &writable1[0];

	std::vector<char> writable2(lesion_text_ac.begin(), lesion_text_ac.end());
	writable2.push_back('\0');
	char* lesion_text_acc = &writable2[0];

	loadImages(lesion_image_acc, lesion_text_acc, 6);
	...
	for (int i = 0; i < test.size(); i++)
	{
		...
		int label[10];
		int index[10];
		char** top_ten = get_top_ten_location(test[i].name.c_str(), test[i].top_x, test[i].top_y, test[i].bottom_x, test[i].bottom_y, label,index);
		Mat top[10];
